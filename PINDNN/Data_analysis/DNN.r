...
impfile = paste0("/work/users/x/i/xiaoyw/DNN_imp_",fold,".csv");
aucfile = paste0("/work/users/x/i/xiaoyw/DNN_auc_",fold,".csv");
prefile = paste0("/work/users/x/i/xiaoyw/DNN_pre_",fold,".csv");
...
#pred[validate, 2] = predict(dnn_mod, newx[validate, ])[, "1"];
pred[, 1] = predict(dnn_mod, newx[validate, ])[, "1"];
print('DNN_mod finished')
...
dfpre = data.frame('PosCT'=oy[validate],'DNN'=pred[,1]);
write.table(dfpre,file=prefile,append=F,row.names=F,col.names=T,sep=",");

dfauc = data.frame(Method = "DNN",
