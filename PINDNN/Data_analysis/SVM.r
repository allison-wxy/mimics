...
impfile = paste0("/work/users/x/i/xiaoyw/SVM_imp_",fold,".csv");
aucfile = paste0("/work/users/x/i/xiaoyw/SVM_auc_",fold,".csv");
prefile = paste0("/work/users/x/i/xiaoyw/SVM_pre_",fold,".csv");
...
#### 2.4 SVM ####
svm_mod = tune.svm(newx[-validate, ], newy[-validate], gamma = 10**(-(0:4)), cost = 10**(0:4/2), tunecontrol = tune.control(cross = fold_num));
svm_mod = svm(newx[-validate, ], newy[-validate], gamma = svm_mod$best.parameters$gamma, cost = svm_mod$best.parameters$cost, probability = TRUE);

# pred[validate, 4] = 1 - attr(predict(svm_mod, newx[validate, ], decision.values = TRUE, probability = TRUE), "probabilities")[, "1"];
pred[, 1] = attr(predict(svm_mod, newx[validate, ], decision.values = TRUE, probability = TRUE), "probabilities")[, "1"];
print('SVM model finished')

#### 3. Summary
## 3.3 Observed and predicted

dfpre = data.frame('PosCT'=oy[validate],'SVM'=pred[,1]);
write.table(dfpre,file=prefile,append=F,row.names=F,col.names=T,sep=",");

dfauc = data.frame(Method = "SVM",
