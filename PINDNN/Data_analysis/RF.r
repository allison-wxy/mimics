...
impfile = paste0("/work/users/x/i/xiaoyw/RF_imp_",fold,".csv");
aucfile = paste0("/work/users/x/i/xiaoyw/RF_auc_",fold,".csv");
prefile = paste0("/work/users/x/i/xiaoyw/RF_pre_",fold,".csv");
...
#### 2.7 RF ####
rf_mod = randomForest(newx[-validate, ], newy[-validate], ntree = n_tree, nodesize = node_size, importance = TRUE);
#pred[validate, 6] = predict(rf_mod, newx[validate, ], type = "prob")[, 2];
pred[, 1] = predict(rf_mod, newx[validate, ], type = "prob")[, 2];
...
dfpre = data.frame('PosCT'=oy[validate],'RF'=pred[,1]);
write.table(dfpre,file=prefile,append=F,row.names=F,col.names=T,sep=",");

dfauc = data.frame(Method = "RF",
